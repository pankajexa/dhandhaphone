...
class SarvamClient {
   ...
   * @param {object} options - { language_code: 'unknown' | 'hi-IN' | 'te-IN' | ... }
  ...
  async synthesize(text, options = {}) {
    ...
    const data = await response.json();
    // v3 returns { audio: "base64..." }, v2 returned { audios: ["base64..."] }
    const audioData = data.audio || (data.audios && data.audios[0]) || '';
    return {
