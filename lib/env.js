...
// Activate network guard — blocks fetch to non-allowed domains
const networkGuard = require('./network-guard');
networkGuard.activate();

// If LLM server is remote (not localhost), add its domain from env
if (process.env.LLM_SERVER_HOST) {
  networkGuard.addAllowedDomain(process.env.LLM_SERVER_HOST);
}
