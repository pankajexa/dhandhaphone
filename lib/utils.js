...
function nowIST() {
  // Use timezone from config if available, fallback to Asia/Kolkata
  let tz = 'Asia/Kolkata';
  try {
    const config = require('./config');
    tz = config.get('timezone') || 'Asia/Kolkata';
  } catch { /* config not available yet — use default */ }
...
function getDB() {
  if (!_db) {
    ...
    _db = new DhandhaDB(path.join(WORKSPACE, 'dhandhaphone.db'));
  }
  return _db;
}

// --- Config convenience accessor ---
let _config = null;
function getConfig() {
...
module.exports = {
  ...
  currentMonth, todayISO, nowIST, generateTxnId, getDB, getConfig
};
